...
def test_voting_power_history(admin, voting_power_history):
    ...
    tx = vph.updateVotingPower(admin, 1e18, 1e18, 0e18)
...
def test_binary_search(admin, voting_power_history):
    vph = voting_power_history
    tx1 = vph.updateVotingPower(admin, 1e18, 1e18, 0e18)
    ...
    chain.sleep(2)
    tx2 = vph.updateVotingPower(admin, 2e18, 1e18, 0e18)
    ...
    assert vph.binarySearch(admin, tx2.timestamp + 10) == (
        True,
        (tx2.timestamp, 2e18, 1e18, 0e18),
    )

    chain.sleep(2)
    tx3 = vph.updateVotingPower(admin, 3e18, 1e18, 0e18)
    # check when the timestamp is after tx3
    assert vph.binarySearch(admin, tx3.timestamp + 10) == (
        True,
        (tx3.timestamp, 3e18, 1e18, 0e18),
    )

...
def test_aggregate_lp_vault_scaled_threshold(admin, aggregate_lp_vault, alice):
    ...
    mv.updateVotingPower(
        alice,
        INITIAL_TOTAL_RAW_VOTING_POWER - INITIAL_RAW_VOTING_POWER,
    )
    ...
    weights = [(mv, 2 * 1e18), (mv2, 1 * 1e18)]
    aggregate_lp_vault.setVaultWeights(weights)
    assert aggregate_lp_vault.getVaultWeights() == weights
